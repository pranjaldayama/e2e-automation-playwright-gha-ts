...
test('Men: Add XS Blue Jacket to cart and checkout with discount', async ({ page }) => {
  ...
  await home.navigateToMenTopsJackets();

  // Apply filters
  const sizeApplied = await listing.applyFilter('Size', testData.men.size);
  const colorApplied = await listing.applyFilter('Color', testData.men.color);
  if (!sizeApplied || !colorApplied) {
    logSkip('No products found for Men Jackets with Size XS and Color Blue');
    test.skip();
  }
  ...
  await details.selectSize(testData.men.size);
  await details.selectColor(testData.men.color);
  await details.setQuantity(testData.men.quantity);
